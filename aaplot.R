...
addKey<- function(from,to,N,maxCor=0.1,corColors){
    ...
    y2 <- (to-from)*0.9+from
    hx <- N/6
    rasterImage(as.raster(matrix(rev(corColors), nrow=1)),0,y0,hx,y1,xpd=T)
    text(0,y2,-maxCor,xpd=T)
    text(hx,y2,maxCor,xpd=T)
    text(N,y0,"evalAdmix\n Correlations",xpd=T,adj=1)
}
...
addCor <- function(corMat,from=1,to=1.2,popID,withinOnly= FALSE,maxCor=0.25,lines=0,x,corColors,meanCor=FALSE){
    ...
    if(lines>0){
        ...
        for(i in 2:length(popSep)-1){
            lines( c( x[popSep[i]+1],x[(N-popSep[i])/2 + popSep[i]]+xSize) , y[c( 1, N-popSep[i])] , xpd=T,lwd=lines )
            lines( c(x[popSepR[i]],(x[popSepR[i]]+xSize)/2) , y[c( 1, popSepR[i])] , xpd=T,lwd=lines )
...
getRel <- function(cm,minKin=0.125){
    ...
    cm[cm<minKin] <- NA
    hist(cm)
    table(  keep <-  apply(cm,1,max,na.rm=T)>0 )
    res <-    data.frame(I1=(1:N)[keep], I2= apply(cm[keep,],1,which.max),cor= apply(cm[keep,],1,max,na.rm=T))
    res
}
