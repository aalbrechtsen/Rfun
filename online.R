...
locusZoomNoLD<-function(pval,w=1,pos,chr,main="LocusZoom",build=37, geneticMap=NA, refGenes=NA){
  if (is.na(geneticMap) && is.na(refGenes)){
      if(build==37 ){
        geneticMap <- "/davidData/data/course/scripts/geneticMap/hg19/genetic_map_GRCh37_chr"
        refGenes <- "/davidData/data/course/scripts/geneticMap/hg19/refGeneHG19.gz"
      } else if(build==38){
       geneticMap <- "/davidData/data/course/scripts/geneticMap/hg38/genetic_map_GRCh38_chr"
        refGenes <- "/davidData/data/course/scripts/geneticMap/hg38/refGeneHG38.gz"
      } else {stop("unknown build")}
  }
...
if(length(start)>0){
    ...
    estart = as.numeric(unlist(sapply(xx2$exonStarts[ord],function(y){strsplit(y,",")[[1]]})))/1e6 #Casia outcomment
    eend = as.numeric(unlist(sapply(xx2$exonEnds[ord],function(y){strsplit(y,",")[[1]]})))/1e6 #Casia outcomment
